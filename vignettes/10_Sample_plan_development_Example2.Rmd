...
# run accumulates cost layer 
acost <- create_accum_cost(x = costprep, origin = start)
names(acost) <- "cost"
terra::plot(acost)
...
pnts <- sf::st_read(list.files(fid$samplingplan_clhs[2], pattern = clhs_set, full.names = TRUE))

tr1 <- terra::rast(file.path(fid$sampling_input_landscape[2], "cost_raw.tif"))

tr1 <- "D:\\PEM_DATA\\DateCreek_AOI\\DateCreek_AOI\\10_clean_inputs\\20_covariates\\25m"

tr11<- readRDS(file.path(tr1, "transition_layer.rds"))
tr11
list.files(tr1)
...
test <- create_vrp(tr11, pnts_in , startLoc, 3)
