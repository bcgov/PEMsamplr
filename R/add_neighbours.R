...
#' @importFrom terra vect ncell adjacent cells
...
add_neighbours <- function(dat_pts, template){
  ...
  cellNums <- terra::cells(template, pts)
  cell_lookup <- data.table(ID = pts$ptsID,cell = cellNums)

  adjCells <- terra::adjacent(template, cells = cellNums[,2],directions = "queen",include = T)
  adjCells <- as.data.table(adjCells)
  setnames(adjCells, c("Orig",paste("Adj",1:8,sep = "")))
  adjCells[,ID := 1:terra::nrow(adjCells)]
  ...
  cellnums <- 1:terra::ncell(template)
  template[!cellnums %in% adjLong$CellNum] <- NA

  pts <- terra::as.points(template,values = T, na.rm = T)
